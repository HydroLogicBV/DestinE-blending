...
def load_and_preprocess_radar(radar_files):
    try:
        importer = io.get_method("knmi_hdf5", "importer")
        radar, _, metadata = io.read_timeseries(
            radar_files,
            importer,
            accutime=5,
            qty="ACRR",
        ...
        log.exception("Input radar data unreadable. Abort script.")
        raise

    radar, metadata = conversion.to_rainrate(radar, metadata)
    del metadata["transform"]
    radar[np.isnan(radar)] = 0

    radar_xr = convert_input_to_xarray_dataset(
        radar,
        None,
        metadata,
        radar_files[1][0],
    ...
    log.debug(
        "Radar time range: %s to %s",
        radar_xr["time"].values[0],
        radar_xr["time"].values[-1],
    )

    return metadata, radar_xr
